...
enum{ clLastScore=20, clBkgnd, clGrid, clBoldGrid, 
...
Tsquare *hitTest(LPARAM p, int *greater=0);
void key(int cmd);
void select(int n);
void stop();
void startTimer();
...
DWORD getTickCount();
TscoreTab *getScoreTab(BYTE gameType, BYTE size, BYTE diag, BYTE killer, BYTE greater, BYTE cons, BYTE oddeven, BYTE flags=0);
TscoreTab *findScoreTab(BYTE gameType, BYTE size, BYTE diag, BYTE killer, BYTE greater, BYTE cons, BYTE oddeven, BYTE flags=0);
void readini();
void writeini();
